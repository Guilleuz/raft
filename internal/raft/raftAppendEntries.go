...
func (nr *NodoRaft) AppendEntry(args *AppendEntryPeticion, reply *AppendEntryRespuesta) error {
	...
	if nr.currentTerm > args.Term || (nr.currentTerm >= args.Term && (nr.estado == CANDIDATO || nr.estado == LIDER)) ||
		...
		// Ignoro entradas si mi mandato es mayor,
		// si la última entrada no es igual
		// o si el mandato es igual y soy líder o candidato
		nr.logger.Printf("Réplica %d: %d entrada(s) de log ignoradas de %d\n",
			nr.yo, len(args.Entries), args.LeaderId)
		reply.Success = false
	} else {
		// Si no, actualizo mi log con las nuevas entradas
		reply.Success = true
...
// Actualiza el log del nodo con las entradas recibidas
func (nr *NodoRaft) actualizarLog(args *AppendEntryPeticion) {
	nr.mux.Lock()
...
func (nr *NodoRaft) gestionarEnvioAppendEntry(nodo int, args AppendEntryPeticion,
	canalMandato chan int, canalSuccess chan Success) {
...
func (nr *NodoRaft) gestionRespuestas(timeout time.Duration, canalMandato chan int,
	canalSuccess chan Success, nuevoNextIndex int, nuevoMatchIndex int) {
...
// Actualización del commitIndex del líder
...
// Sometemos las nuevas entradas comprometidas a la máquina de estados
